...
class DelayTaskTest {
    ...
    void testDelayQueue() throws InterruptedException {
        ...
        //2.向队列中添加延迟执行的任务
        log.info("开始初始化延迟任务...");
        queue.add(new DelayTask<>("延迟队列3", Duration.ofSeconds(3)));
        queue.add(new DelayTask<>("延迟队列1", Duration.ofSeconds(1)));
        queue.add(new DelayTask<>("延迟队列2", Duration.ofSeconds(2)));
